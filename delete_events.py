...
id_eventos = """select q. *, s.eventid FROM ultimo_saldo_sku q, stg_evento_sl_catalyst s
WHERE q.id_stg_evnt_sl_ctlyst = s.id_stg_evnt_sl_ctlyst AND
TRIM(q.sku) IN"""

borrar_eventos = "delete from ultimo_saldo_sku where id_ultimo_saldo_sku in {sku}"
...
with oracledb.connect(user=username, password=userpwd, dsn=dsn) as conn:
    cursor = conn.cursor()
    res = cursor.execute(f"{id_eventos} {sku_tuple}").fetchall()
    if len(res) > 0:
        for line in res:
        ...
        cursor.execute(borrar_eventos.format(sku=events_tuple))
